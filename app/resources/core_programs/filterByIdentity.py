...
with open(tempFolder + "/Filtered/Filtration_Log.txt", "w") as logFile:
    ...
    for filename in os.listdir(tempFolder + "/BLAST"):  # For every file
        if "_vs_" in filename and filename.endswith(".txt"):  # Interprets identity from strain v strain BLAST
            ...
            thisStrain = ".".join(filename.split(".")[0:len(
                filename.split(".")) - 1])  # This nonsense to allow for strain names with periods in them
            strains.append(thisStrain)

    # Printing for testing
    with open(tempFolder + "/Filtered/Statistics.txt", "w") as f:
        dataCount = 0
        for versus in avgIdentityData.keys():
            ...
            if twoStrainArr[0] not in strains or twoStrainArr[1] not in strains:
                ...
                logFile.write(str(dataCount) + " of " + str(
                    len(avgIdentityData.keys())) + "- BLAST data involving unused strain discarded: " + versus + "\n")
            else:
                f.write(str(dataCount) + " of " + str(len(avgIdentityData.keys())) + "- " + versus + "\t" + str(
                    avgIdentityData[versus]) + "\n")
    ...
    with open(tempFolder + "/Filtered/RemovalForSimilarity.txt", "w") as f:
    ...
    if len(strains) >= 3:  # Ceases calculations if less than 3 strains
        ...
        for strain in strains:  # For each remaining strain
            ...
            if numpy.isnan(float(avgOfStrainAvgs)):  # Occasional NaN issue with avgOfStrainAvgs? How to handle?
                ...
                logFile.write(strain + " Average of Strain-Vs-Other-Strain Average Identities: " + str(
                    avgOfStrainAvgs) + ", which is under the critical value by " + str(
                    lowerBound - avgOfStrainAvgs) + "\n")
        ...
        with open(tempFolder + "/Filtered/RemovalForDissimilarity.txt",
                  "w") as f:  # "Mean AvgIdentity: " + str(sampleMean)
            f.write("Mean AvgIdentity: " + str(sampleMean) + "\tStandard Deviation: " + str(
        ...
        logFile.write(
            "Remaining " + str(len(strains)) + " " + specLabel + " strains recognized (final): " + str(strains) + "\n")
