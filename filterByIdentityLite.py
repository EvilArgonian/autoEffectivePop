...
with open("temp/" + specLabel + "/Filtered/Filtration_Log_Lite.txt", "a+") as logFile:
    ...
    for filename in os.listdir(folder):# For every file
        fileNameFormatted = ".".join(filename.split(".")[0:len(filename.split(".")) - 1])  # This nonsense to allow for strain names with periods in them
    ...
    if sampleSize == 1: # One versus file, or just 2 strains
        ...
        with open("temp/" + specLabel + "/Filtered/Output.txt", "w") as outFile:
            outFile.write(" ".join(strains)) # Cannot do any statistical evaluation, so the strains are simply passed through
        sys.exit()
